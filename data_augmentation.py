...
def segmentation_reconstruction(data, labels, num_segments=8, num_augmentations=3):
    ...
    for label in type_labels:
        ...
        num_samples_for_classes = data_for_label.shape[0]*num_augmentations
        label_tmp = labels[idx_labels][0]
        new_data = torch.zeros((num_samples_for_classes, conv, ch, time), device=data.device)
        for i in range(num_samples_for_classes):
            for seg_idx in range(num_segments):
                # Selezioniamo casualmente un campione dal batch originale
